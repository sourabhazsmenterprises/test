...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{url("admin/users/update", [$user->id]) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
            @csrf
            @method('PUT')
            <div class="row">
            ...
            <div class="col-md-6 col-lg-6 col-sm-6 form-group {{ $errors->has('email') ? 'has-error' : '' }}">
              ...
              <div class="col-md-6 col-lg-6 col-sm-6 form-group {{ $errors->has('mobile') ? 'has-error' : '' }}">
                ...
                <input type="mobile" id="mobile" name="mobile" class="form-control" value="{{ old('mobile', isset($user) ? $user->mobile : '') }}">
            ...
            <div class="col-md-6 col-lg-6 col-sm-6 form-group {{ $errors->has('password') ? 'has-error' : '' }}">
             ...
             <div class="col-md-6 col-lg-6 col-sm-6 form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                ...
                <select name="type" id="roles" required class="form-control" style="width:510px;">
                         <option value="1" @if($user->type=='1') selected @endif)>Patient </option>
					<option value="2"  @if($user->type=='2') selected @endif>Doctors </option>
                     <option value="3" @if($user->type=='3') selected @endif>Diagnostic Centers  </option>
					<option value="4" @if($user->type=='4') selected @endif>Pharmacy</option>
                </select>
                @if($errors->has('roles'))
                    <em class="invalid-feedback">
                        {{ $errors->first('roles') }}
